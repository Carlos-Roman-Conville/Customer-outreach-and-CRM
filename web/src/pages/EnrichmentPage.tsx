...
export function EnrichmentPage() {
  ...
  const startMut = useMutation({
    mutationFn: (kind: 'enrich' | 'verify') => api.startEnrich(kind),
  ...
  return (
    <div>
      ...
      <p style={{ color: 'var(--text-muted)' }}>Email discovery and MX verification progress</p>
      ...
      <div className="card" style={{ marginTop: '1rem' }}>
        ...
        <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '1rem' }}>
          Verification must reach 100% before email batch export is allowed.
        </p>
      </div>
    </div>
  );
